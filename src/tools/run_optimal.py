"""Optimal-params backtest — re-run the v3 alpha screen with fresh seed.

Identifies the "optimal" parameter set from the surviving alpha candidates
(after the 2026-09-17 look-forward-bias cleanup) and runs them on N
Mon-Fri UTC days sampled from the 1y XAUUSD corpus.

Configs tested:
  1. BASELINE       — v2 defaults (no INV_TRADE, no rolling tier)
  2. INV_TRADE      — fvg_inv_trade_enabled=True (best single-knob from v3 study)
  3. INV_TRADE_PLUS — INV_TRADE + RollingFvgRanker tier treatment (never A/B'd)

Output:
  - notebooks/optimal_per_day.csv   per-day breakdown
  - notebooks/optimal_summary.csv   per-config totals
  - stdout summary table

Usage:
  python src/tools/run_optimal.py [N_DAYS]
"""
...
RNG_SEED = 20260917  # fresh seed, distinct from alpha_v2's 20260915
...
def _common() -> TrendStrategyParams:
    """v2 BASELINE defaults — the most conservative causal config."""
    return TrendStrategyParams(
        ...
        # BoS/CHoCH alignment soft-stop suppression (added 2026-09-16).
...
CONFIGURATIONS = {
    # 1. BASELINE — v2 defaults, no knobs flipped
    "BASELINE": {},
    # 2. INV_TRADE — best single-knob improvement from v3 study
    "INV_TRADE": {"fvg_inv_trade_enabled": True},
    # 3. INV_TRADE_PLUS — INV_TRADE + RollingFvgRanker causal tier treatment
    "INV_TRADE_PLUS": {
...
def _run_one(cfg_name: str, day_label: str, df_day: pd.DataFrame) -> dict:
    ...
    try:
        with contextlib.redirect_stdout(buf):
            res = run_ict_backtest(df_day, p, strategy_label=f'optimal_{cfg_name}')
...
csv_path = nb_dir / 'optimal_per_day.csv'
...
summary_path = nb_dir / 'optimal_summary.csv'
summary.to_csv(summary_path, index=False)
print(f'Summary CSV: {summary_path}')

# ── Pretty-print ────────────────────────────────────────────────────────
print('\n' + '=' * 110)
print(f'OPTIMAL PARAMS BACKTEST — {len(day_data)} Mon-Fri UTC days, seed {RNG_SEED}')
print('=' * 110)
hdr = (f'{"config":<20} {"trades":>7} {"EV":>10} {"PnL/day":>10} '
...
for _, row in summary.iterrows():
    ...
    print(f'{row["config"]:<20} {int(row["n_trades"]):>7} '
...
print(f'  {"config":<20} {"dEV/trade":>10} {"dPnL/day":>10} {"dtrades":>8} {"positive_EV":>10}')
results = []
for _, row in summary.iterrows():
...
for r in results:
    ...
    print(f'  {r["config"]:<20} ${r["delta_ev_per_trade"]:>+9.4f} '
